...
async function newIcon(svgMaster, file, sprite, name) {
  ...
  const optimizeContent = astTree.data
...
async function writeJsonInfo(directory) {
  ...
  await fs.writeFile(directory + "/sprites.json", JSON.stringify(json, 0, 2), {
    flag: "w"
  })
}
...
async function writeSprite(svgMaster, name, directory) {
  ...
  await svgMaster.writeSVG(join(directory, `${name}.svg`), svg)
}

export async function addIcon(
  svgMaster,
  file,
  { input, output, defaultSprite, ...options }
) {
  ...
  await writeIconStylesFile(svgMaster, options)
...
export async function invalidateIcon(
  svgMaster,
  file,
  { input, output, defaultSprite, ...options }
) {
...
export async function invalidateSprite(
  svgMaster,
  file,
...
export async function writeIconStylesFile(svgMaster, options) {
  ...
  await fs.writeFile(options.styleFilePath, content.join("\r\n"), {
    flag: "w"
  })

  logger.log({
    type: "changed",
    message: `SVG styles generated.`,
    icon: chalk.green.bold("*")
  })
}
